...
trait Readable[A] {
  ...
  def readData(a: A,
               ...
               to: Option[LocalDateTime] = None): Either[String, DataFrame]
}

object Readable {

  def genericReadDataSource(a: DataSource): Either[String, DataFrame] = {
    Try(SparkSession.active.read.options(a.readOptions).format(a.format.sparkFormat).load(a.location))
      .toEither
      .leftMap(_.getMessage)
  }

  def genericReadScd2Table(a: Scd2Table, to: Option[LocalDateTime]): Either[String, DataFrame] = {
    val result: Either[String, DataFrame] = genericReadDataSource(a)
    to.fold {
      result
    }{datetime =>
      result
  ...
  def genericReadFactTable(a: FactTable, from: Option[LocalDateTime], to: Option[LocalDateTime]): Either[String, DataFrame] = {
    val data = genericReadDataSource(a)
    (from, to) match {
  ...
  implicit val deltaTableScd2CanBeRead: Readable[DeltaScd2Table] = new Readable[DeltaScd2Table] {
    override def readData(a: DeltaScd2Table, from: Option[LocalDateTime], to: Option[LocalDateTime]): Either[String, DataFrame] = {
      genericReadScd2Table(a, to)
    }
  }

  implicit val deltaTableScd1CanBeRead: Readable[DeltaScd1Table] = new Readable[DeltaScd1Table] {
    override def readData(a: DeltaScd1Table, from: Option[LocalDateTime], to: Option[LocalDateTime]): Either[String, DataFrame] = {
      genericReadDataSource(a)
    }
  }

  implicit val deltaTableInsertCanBeRead: Readable[DeltaInsertTable] = new Readable[DeltaInsertTable] {
    override def readData(a: DeltaInsertTable, from: Option[LocalDateTime], to: Option[LocalDateTime]): Either[String, DataFrame] = {
      genericReadFactTable(a, from, to)
    }
  }

  implicit val deltaTableUpsertCanBeRead: Readable[DeltaUpsertTable] = new Readable[DeltaUpsertTable] {
    override def readData(a: DeltaUpsertTable, from: Option[LocalDateTime], to: Option[LocalDateTime]): Either[String, DataFrame] = {
      genericReadFactTable(a, from, to)
    }
  }
  ...
  implicit class ReadableImplicits[B](spark: SparkSession) {
    def readData(b: B,
                 from: Option[LocalDateTime] = None,
                 to: Option[LocalDateTime] = None)(implicit read: Readable[B]): Either[String, DataFrame] = {
